...
function displayPersonalAnalysis(abilityIndexes, ministryScores) {
  ...
  const path = [
    { label: "現在開始", value: ministryScores[0].name },
    { label: "累積經驗", value: ministryScores[1].name },
    { label: "延伸挑戰", value: ministryScores[2].name }
  ];
  elements.growthPath.innerHTML = path.map((item, index) => `
    <div class="growth-step"><span>${index + 1}</span><small>${item.label}</small><strong>${item.value}</strong></div>
